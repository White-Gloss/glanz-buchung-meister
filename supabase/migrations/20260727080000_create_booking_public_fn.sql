-- SECURITY DEFINER function for public (anon) booking creation.
-- All financial values (total, deposit, invoice number) are computed server-side
-- from canonical data in service_prices. Callers provide only service IDs and
-- customer data – no amounts accepted from the caller.
...
-- Grant execute to anon (called via server function using the publishable key)
-- and to authenticated/service_role for admin or fallback use.
