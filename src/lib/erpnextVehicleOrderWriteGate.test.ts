...
describe("ERPNext vehicle/order production write gate", () => {
  ...
  it("requires an active one-time approval", () => {
    ...
    expect(result.error).toBe("production_write_approval_required");
  });
  ...
  it("issues a server-signed confirmation for one booking revision", async () => {
    ...
    expect(confirmation).toMatch(/^WGVO4\./);
    await expect(
      verifyVehicleOrderWriteConfirmation({
        secret: SECRET,
        bookingId: BOOKING_ID,
  ...
  it("rejects a missing, wrong or replaced approval id", async () => {
    const confirmation = await issueConfirmation();
    const unboundConfirmation = await issueVehicleOrderWriteConfirmation({
      secret: SECRET,
      bookingId: BOOKING_ID,
      bookingRevision: BOOKING_REVISION,
      approvalId: null,
