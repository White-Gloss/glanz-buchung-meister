...
1. Abhängigkeiten installieren und den vorhandenen Node-Produktionsbuild erstellen. Wie in der CI `GITHUB_ACTIONS=true` setzen und `npm run build` ausführen. Für diesen isolierten Build `DATABASE_URL` leer lassen, damit der bestehende Build-Migrationsschritt keine externe Datenbank anspricht.
