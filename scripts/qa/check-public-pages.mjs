...
const base = process.env.FRONTEND_BASE_URL || qaBase;
...
for (const path of paths) {
  ...
  const canonicals = [...head.matchAll(/<link\b[^>]*rel="canonical"[^>]*>/g)].map((m) =>
    attr(m[0], "href"),
  );
  ...
  for (const [, level] of html.matchAll(/<h([1-6])\b/g)) {
    if (Number(level) > previousHeading + 1)
      found.push(`Heading level jumps from ${previousHeading} to ${level}`);
  ...
  if (!metas.some((m) => attr(m, "name") === "description" && attr(m, "content")))
    found.push("Missing description");
  for (const property of ["og:title", "og:description", "og:image", "og:url"]) {
    if (!metas.some((m) => attr(m, "property") === property && attr(m, "content")))
      found.push(`Missing ${property}`);
  }
  ...
  for (const [link] of html.matchAll(/<a\b[^>]*>/g)) {
    ...
    if (
      ...
      !["noopener", "noreferrer"].every((v) => (attr(link, "rel") || "").split(/\s+/).includes(v))
    )
      found.push(`Unsafe external tab: ${href}`);
    if (href?.startsWith("/") || href?.startsWith("#"))
      links.add(new URL(href.replaceAll("&amp;", "&"), new URL(path, base)).href);
  }
...
for (const href of links) {
  const url = new URL(href),
    ids = idsByPath.get(url.pathname.replace(/\/$/, "") || "/");
  if (url.hash && ids && !ids.has(decodeURIComponent(url.hash.slice(1))))
    anchorIssues.push(url.pathname + url.hash);
}
const report = {
  base,
  ...
  issues,
  anchorIssues: [...new Set(anchorIssues)],
  pages,
};
  ...
  JSON.stringify(
    {
      ...
      internalLinks: links.size,
      issues,
      anchorIssues: report.anchorIssues,
    },
    null,
    2,
  ),
